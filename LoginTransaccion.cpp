...
using namespace std;
LoginTransaccion::LoginTransaccion(wxWindow *parent, GestionHabitaciones *m_agendaHabitaciones, GestionTransacciones *m_transacciones): 
		LoginTransacciones(parent), m_agendaHabitaciones(m_agendaHabitaciones), m_transacciones(m_transacciones){
...
void LoginTransaccion::ClickBotonIngresarTransacciones( wxCommandEvent& event )  {
	...
	if(m_transacciones->verContrasena() == x){
		Transacciones *win = new Transacciones(this,m_agendaHabitaciones,m_transacciones);
		win -> Show();
	}
	else{
